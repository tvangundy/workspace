...
# Source shared libraries (bin/vm/lib when this file is in bin/vm/scripts/runner)
...
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "  VM: ${TEST_REMOTE_NAME}:${VM_INSTANCE_NAME}"
echo "  User: ${RUNNER_USER}"
echo ""

# Check if runner is installed
RUNNER_INSTALLED=false
if incus exec "${TEST_REMOTE_NAME}:${VM_INSTANCE_NAME}" -- test -f "${RUNNER_HOME}/actions-runner/svc.sh" 2>/dev/null; then
  RUNNER_INSTALLED=true
fi

if [ "${RUNNER_INSTALLED}" = "false" ]; then
  echo "❌ GitHub Actions runner is not installed"
  echo "   Run: task vm:instantiate -- <remote-name> <remote-ip> ${VM_INSTANCE_NAME} --runner"
  exit 1
fi

# Get service status
echo "  Service Status:"
incus exec "${TEST_REMOTE_NAME}:${VM_INSTANCE_NAME}" -- bash -c "
...
if incus exec "${TEST_REMOTE_NAME}:${VM_INSTANCE_NAME}" -- test -f "${RUNNER_HOME}/actions-runner/.runner" 2>/dev/null; then
  # Parse JSON on the VM using jq (which should be installed)
  RUNNER_NAME_CONFIG=$(incus exec "${TEST_REMOTE_NAME}:${VM_INSTANCE_NAME}" -- bash -c "jq -r '.agent.name // empty' ${RUNNER_HOME}/actions-runner/.runner 2>/dev/null" || echo "Unknown")
  RUNNER_URL=$(incus exec "${TEST_REMOTE_NAME}:${VM_INSTANCE_NAME}" -- bash -c "jq -r '.serverUrl // empty' ${RUNNER_HOME}/actions-runner/.runner 2>/dev/null" || echo "Unknown")
  ...
  if [ "${RUNNER_NAME_CONFIG}" = "Unknown" ] || [ -z "${RUNNER_NAME_CONFIG}" ]; then
    ...
    RUNNER_CONFIG=$(incus exec "${TEST_REMOTE_NAME}:${VM_INSTANCE_NAME}" -- cat "${RUNNER_HOME}/actions-runner/.runner" 2>/dev/null || echo "")
