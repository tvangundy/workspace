...
if [ ${CONFIG_EXIT} -ne 0 ]; then
  ...
  if echo "${CONFIG_OUTPUT}" | grep -qi "runner exists with the same name"; then
    ...
    echo ""
    echo "  The token and repository URL must match exactly:"
    echo "  - For a REPOSITORY runner: generate the token at the link below (same repo as REPO_URL)."
    echo "  - If you used an ORGANIZATION runner token, use org URL instead:"
    echo "    https://github.com/organizations/YOUR_ORG"
    echo "  - Registration tokens expire in ~1 hour and are single-use; generate a fresh one."
    echo ""
    echo "  To fix this:"
    ...
    echo "  1. Open this exact page (for this repository):"
    echo "     https://github.com/${REPO_PATH}/settings/actions/runners/new"
    echo "  2. Generate a new registration token and use it immediately"
    echo "  3. Update secrets: GITHUB_RUNNER_TOKEN: <new-token> in contexts/.../secrets.yaml"
    echo "  4. Re-run this task"
    echo ""
    echo "  Repository URL used: ${REPO_URL}"
