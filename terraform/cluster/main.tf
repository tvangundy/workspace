  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      if [ -n "$${MISSING_IPS}" ]; then
        ...
        echo ""
        echo "  1. Open the Incus Web UI: https://<incus-host-ip>:8443"
        echo "  2. Wait for the nodes to complete booting to Talos (check the console)"
        echo "  3. Find the DHCP-assigned IP address for each VM:"
        echo "     - Control plane: ${incus_instance.control_plane.name}"
