...
description: "Step-by-step recipes for building deployments from the ground up"
---
# Deployment Recipes

Welcome to the deployment recipes section! This folder contains comprehensive, step-by-step guides that walk you through building complete deployments from the ground up. Each recipe is designed to help new users understand and implement the infrastructure patterns demonstrated in the [examples folder](../examples/).

## Purpose

The deployment recipes serve as instructional guides that break down the implementation process into clear, actionable steps. While the [examples folder](../examples/) contains working reference implementations that are used for regression testing and serve as the "final product," the deployment recipes focus on the **how** and **why** behind each step, making them ideal for:
...
## How Recipes Relate to Examples

The examples and deployment recipes work together:

- **Examples folder** (`/examples/`): Contains production-ready, tested implementations that serve as reference code and are used for regression testing. These are the "what" - the complete, working solution.

- **Deployment recipes** (`/docs/deployments/`): Provide the "how" - detailed, step-by-step instructions that guide you through building a deployment that matches the corresponding example implementation.

By following a deployment recipe, you'll end up with a deployment that implements the same patterns and achieves the same results as the corresponding example in the examples folder.


## Available Deployment Recipes

The following deployment recipes are available:
...
### 🏃 [GitHub Actions Runners](./runners/)
Step-by-step guides for setting up self-hosted GitHub Actions runners on various platforms. These recipes cover installing and configuring runners on Windows, Ubuntu, and macOS systems, enabling you to run CI/CD workflows on your own infrastructure.
...
Additional deployment recipes are being developed for:
...
2. **Choose a recipe**: Select a deployment recipe that matches your needs

3. **Follow step-by-step**: Each recipe provides detailed instructions from initial setup through final deployment
...
- **Read the entire recipe first**: Understanding the full process before starting helps avoid common pitfalls
