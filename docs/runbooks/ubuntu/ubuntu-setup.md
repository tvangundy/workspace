...
**Single disk** (default):
```bash
task device:write-ubuntu-iso
```

**Multiple disks**: To write the ISO to multiple disks simultaneously, specify the total number of disks. For example, to write to 2 disks starting from the disk specified in `USB_DISK`:
...
If you need to completely wipe the NUC's boot disk to start fresh, you can use an Ubuntu Live USB to boot into a recovery environment and wipe the disk. See the [Incus Setup runbook](../incus/incus-setup.md) for detailed instructions on creating an Ubuntu Live USB and wiping disks.
