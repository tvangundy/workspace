# Remote Development Environment on IncusOS

This runbook guides you through creating and managing development environments (VMs or containers) on a remote IncusOS server. This setup provides a standalone remote development environment ideal for persistent development work with direct SSH access.

## Overview

Remote development environments run on an IncusOS server and provide:
- Isolated, reproducible development environments
- Persistent storage on the remote server
- **Direct SSH access** from your local network (when using VMs)
- Access via `incus exec` (similar to `docker exec`)
- Ability to initialize with workspace contents on creation
- File transfer capabilities between local workspace and remote instance
...
environment:
  ...
  DEV_INIT_WORKSPACE_PATH: "/workspace"  # Path in container where workspace will be initialized
...
Before creating a container, verify you can connect to the remote Incus server:
...
### Step 3: Create the Development Instance

Create a VM on the remote server (recommended for direct SSH access):
...
#### Alternative: Create a Container (Lighter Weight, but Limited Network Access)

If you don't need direct SSH access, you can create a container instead:

```bash
task dev:create -- container $DEV_IMAGE
```

**Limitations:**
- Gets an IP from the Incus bridge network (typically `10.x.x.x`)
- May not be directly accessible via SSH from your local machine
- Can still access via `task dev:shell` or SSH through the Incus host

**When to use**: If you don't need direct SSH access and are comfortable using `task dev:shell` or SSH'ing through the Incus host.

The VM will:
- Create an instance named `dev-container` (or the name in `DEV_INSTANCE_NAME`)
...
- Configure the instance with the specified resources
...
### Step 4: Access the Development Instance

You can access the instance in three ways:
...
This is equivalent to `docker exec -it <instance> bash`. You'll get a shell prompt where you can run commands interactively.
...
# Or SSH directly into the workspace directory
ssh $(whoami)@<vm-ip> -t 'cd /workspace && bash'
```
...
- **Workspace directory** - Located at `/workspace` with proper permissions
...
# Push to workspace directory
incus file push local-file.txt nuc:dev-container/workspace/
```
...
incus file push -r ./src nuc:dev-container/workspace/

# Push with specific permissions
incus file push -r --mode=0755 ./scripts nuc:dev-container/workspace/
```

#### Push from Workspace Root

```bash
# Push entire workspace to VM
incus file push -r "${WINDSOR_PROJECT_ROOT}/" nuc:dev-container/workspace/
```

### Transferring Files from Container
...
# Pull from workspace directory
incus file pull nuc:dev-container/workspace/output.txt ./
```
...
incus file pull -r nuc:dev-container/workspace/output ./

# Pull to specific destination
incus file pull -r nuc:dev-container/workspace/build ./local-build/
```
...
```bash
# Sync local workspace to remote container
rsync -avz --exclude='.git' "${WINDSOR_PROJECT_ROOT}/" user@nuc:/tmp/workspace-sync/
incus file push -r /tmp/workspace-sync/ nuc:dev-container/workspace/

# Or use incus file push directly (slower but simpler)
incus file push -r "${WINDSOR_PROJECT_ROOT}/" nuc:dev-container/workspace/
```

**Note**: For large workspaces, consider using `rsync` over SSH to the remote server first, then using `incus file push` from the server to the container.
...
```bash
# SSH into VM (get IP from task dev:ssh-info)
ssh $(whoami)@<vm-ip> -t 'cd /workspace && bash'
...
- [ ] Workspace was initialized (if enabled) (`task dev:exec -- ls -la /workspace` shows files)
...
- [ ] Can clone repositories (`task dev:exec -- git clone git@github.com:user/repo.git /workspace/test` works)
...
- [ ] **Final test**: SSH into VM, navigate to `/workspace`, and run tasks (e.g., `task help`)
...
incus file pull -r nuc:dev-container/workspace/ "${WINDSOR_PROJECT_ROOT}/"
```
...
incus file pull -r nuc:dev-container/workspace/project "${WINDSOR_PROJECT_ROOT}/"
```
